 ...
 * `isPrivateHost` from the gateway helpers is composed on top as a second
 * check, so a future allow-list entry that resolves somewhere internal is
 * still refused, and so the three SSRF checks in this repo share one notion
 * of "private".
 *
 * This module deliberately holds no Prisma, `jszip` or `server-only` import,
 * so a route handler can pull in the guard without those in its bundle.
 */
