...
As of session 2 (2026-07-02): all 9 numbered High/Medium findings from the original backlog (§5) are fixed and independently re-verified live; a deeper follow-on bug in split-expense (found via a genuine regression, not part of the original 9) is fixed. A suspected new Medium-severity bug (cashflow projection returning identical 30/60/90-day figures) was raised by the session-2 closing audit but, on tracing the actual code against live data, turned out not to be a bug — see §5. **Net: no known Critical, High, or Medium-severity defect remains open.**
...
| ~~Cashflow projection returns identical figures for 30/60/90-day windows~~ | **Investigated and closed — not a bug.** The session-2 closing audit flagged identical 30/60/90-day figures as a methodology bug, theorizing the backend ignores due dates. Traced the actual code (`/api/v1/agentbook-tax/cashflow/projection/route.ts`) against Maya's live invoice data instead of assuming: the endpoint correctly buckets `sent`/`viewed` invoices by `dueDate` against each window, and deliberately treats `overdue` invoices as payable "any time from now" (`expectedPayDate = now`, so they qualify for every window — a defensible modeling choice, not a bug). Maya's 10 outstanding invoices all happen to have an expected-pay-date ≤30 days out (4 `sent` due in 25-29 days, 6 `overdue`) — there's simply no invoice in the 31-90 day range in this test tenant's data, so all three windows legitimately converge on the same total. `recurringExpenses: []` for the same reason — no active recurring rules for Maya, so $0 is correct. Verified live via the endpoint's own JSON response, not just the rendered page. No code change made. |
...
- **Suspected finding, later closed**: the cashflow projection endpoint returned identical figures for all three time windows, initially read as a methodology bug — see §5 for why a closer trace against live data showed this was a coincidence of the test tenant's invoice dates, not a code defect.
...
All 4 notification PRs are now merged (#180, #181, #187, #192) and the calendar-check cron's fix confirmed healthy over 12 consecutive hourly production runs. The Plaid fix (#189) was independently spot-checked and its recovery path confirmed robust (5/5 clean recoveries across 3 sandbox institutions). The suspected cashflow-projection bug was traced and closed as not-a-bug (§5). Combined with the full QA-fix sprint (§8.1) and split-expense saga (§8.3), **no known Critical, High, or Medium-severity defect remains open.** Recommend: **unconditional go**, with one fast-follow — a full Plaid connect-and-persist happy-path run (this session's spot-check confirmed recovery but not end-to-end persistence, due to testing-tool timing, not an app defect).
