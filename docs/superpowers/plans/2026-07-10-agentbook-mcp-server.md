...
> **Superseded during Task 8 implementation:** the `sessionIdGenerator: undefined` (stateless) transport shown above turned out to make elicitation (Task 8) structurally impossible for *any* client — a client's reply to `elicitation/create` arrives as a new HTTP request routed to a fresh, unrelated `McpServer` instance with no memory of the pending request, so it always times out. Fixed by switching to stateful mode (`sessionIdGenerator: () => randomUUID()`) with a module-level session store (`apps/web-next/src/app/api/v1/mcp/session-store.ts`) that reuses the same `McpServer`/transport instance across requests carrying a known `Mcp-Session-Id`, re-validates the requesting tenant against the session's creating tenant on every reuse (destroying the session on mismatch — this was a real cross-tenant exposure risk the naive rework introduced and a review caught), and evicts idle sessions after 30 minutes. Correctness now depends on Vercel routing a session's follow-up request to the same warm instance (Fluid Compute reuses instances across requests, workable for a short human-confirmation round-trip, though not an absolute guarantee — an instance recycling mid-wait causes a timeout, not a silent wrong answer). This also required fixing three latent bugs in Task 4a's `node-web-adapter.ts` that blocked the rework: missing `rawHeaders` (the SDK read zero headers from the adapted request), `writeHead()` bypassing the response's header capture, and whole-response buffering (which would deadlock any real elicitation round-trip needing a partial flush before the client can reply).
