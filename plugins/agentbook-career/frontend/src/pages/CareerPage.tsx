...
export const CareerPage: React.FC = () => {
  ...
  const runDiscover = async () => {
    ...
    setSearchNote(null);
    try {
      const res = await careerApi.discover(query.trim() || undefined);
  ...
  const setStatus = async (id: string, status: string) => {
    ...
    try { await careerApi.setStatus(id, status); } catch { await load(); }
  };

  const remove = async (id: string) => {
    setItems((xs) => xs.filter((x) => x.id !== id));
    try { await careerApi.remove(id); } catch { await load(); }
  };
    ...
    <div className="max-w-4xl mx-auto p-4 sm:p-6">
      <div className="mb-5">
        <h1 className="text-xl font-bold text-foreground flex items-center gap-2">
          <Briefcase className="w-5 h-5" /> Career &amp; Co-op Copilot
        </h1>
        <p className="text-sm text-muted-foreground mt-0.5">
          Find student jobs and co-op placements, then track and prepare each application.
          AgentBook finds and helps you prepare — <strong>you review and apply</strong>.
        </p>
      </div>

      {/* Discover */}
      <div className="rounded-xl border border-border bg-card p-4 mb-6">
        <label className="block text-sm font-medium text-foreground mb-2">Find jobs &amp; co-ops</label>
        <div className="flex gap-2">
          <input
            className={inputCls}
            placeholder="Optional focus — e.g. &ldquo;software co-op&rdquo; or &ldquo;on-campus part-time&rdquo;"
            value={query}
      ...
      {loading ? (
        ...
        <p className="text-sm text-muted-foreground py-6 text-center rounded-lg border border-dashed border-border">
          Nothing tracked yet — search above, or track a posting you&apos;ve already found.
        </p>
      ) : (
            ...
            <div key={o.id} className="rounded-lg border border-border bg-card p-3 flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <div className="mt-0.5 text-xs text-muted-foreground flex flex-wrap items-center gap-x-3 gap-y-0.5">
                  {o.payload?.employer && <span>{o.payload.employer}</span>}
                  {o.payload?.location && <span className="inline-flex items-center gap-1"><MapPin className="w-3 h-3" />{o.payload.location}</span>}
                  {o.payload?.compText && <span>{o.payload.compText}</span>}
                ...
                {o.payload?.summary && <div className="mt-1 text-xs text-muted-foreground">{o.payload.summary}</div>}
              </div>
