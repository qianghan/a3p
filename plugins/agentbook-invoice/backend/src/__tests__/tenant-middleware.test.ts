...
//
// This plugin previously had NO req.user check at all — it trusted the
// client-supplied x-tenant-id header unconditionally in every
// environment, including production. The cases below assert req.user.id
// is now required in production (401 without it) and wins over any
// (spoofed) header when present.
...
describe('agentbook-invoice tenant middleware — production mode', () => {
  beforeEach(() => {
...
describe('agentbook-invoice tenant middleware — development mode', () => {
  beforeEach(() => {
