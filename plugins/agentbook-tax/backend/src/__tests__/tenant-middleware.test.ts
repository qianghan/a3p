...
//
// This also closes a priority-ordering bug in this plugin's previous
// getTenantId() helper: it checked the client-supplied x-tenant-id
// header BEFORE req.user?.id, letting an authenticated attacker spoof
// the header to impersonate a tenant they don't own. The cases below
// assert req.user.id wins even when a (spoofed) header is present.
...
describe('agentbook-tax tenant middleware — production mode', () => {
  beforeEach(() => {
...
describe('agentbook-tax tenant middleware — development mode', () => {
  beforeEach(() => {
