...
export function renderFilingPDF(filing: any, forms: Record<string, any>, templates: any[]): string {
  ...
  const facts = jurisdictionFacts(filing.jurisdiction);
...
<style>
  ...
  for (const template of templates) {
    ...
    for (const section of (template.sections || [])) {
      ...
      for (const field of (section.fields || [])) {
        ...
        // Was a bare `$` for every jurisdiction, so a Canadian filing showed
        // US dollars on the document it is filed from. Same defect class as
        // the chat money bugs (#482/#499) — fixed the same way, from the
        // filing's own jurisdiction rather than a hardcoded symbol.
        const displayValue = field.type === 'currency'
          ? formatCurrency(value || 0, facts.locale, facts.currency)
          : field.type === 'percent'
          ? `${value || 0}%`
          : String(value || '—');
  ...
  // Hardcoded "CRA" before this, so a US or Australian filer's worksheet named
  // the wrong revenue agency. It also claimed only "for review purposes", which
  // understates the point: AgentBook is not an authorised e-file transmitter in
  // any jurisdiction, and the reader needs to know that this is a worksheet to
  // key in or hand over, not something to submit.
  html += `<div class="footer">Generated by AgentBook — a worksheet, not an official ${facts.agency} document. ` +
    `Line numbers refer to the official ${facts.agency} forms; complete your return at ${facts.portal} or give this to your accountant.</div>`;
  html += `</body></html>`;
  return html;
}
...
export async function exportFiling(
  tenantId: string, taxYear: number, format: 'pdf' | 'json' | 'csv',
  ...
  if (format === 'csv') {
    // The reviewed-or-not fact is stamped INTO the file, not just returned
    // alongside it, because the file is what gets forwarded to an accountant
    // and the API response is not. `hasConfirmedFreshReview` already existed
    // in tax-review-agent.ts and nothing called it from the export path, which
    // is why "reviewed e-file export" was absent end to end.
    const reviewed = await hasConfirmedFreshReview(tenantId, taxYear).catch(() => false);
    const facts = jurisdictionFacts(filing.jurisdiction);
    const rows = buildWorksheetRows(forms, templates);
    ...
    await db.abTaxFiling.update({
      where: { id: filing.id },
      data: { exportData: { format: 'csv', rows: rows.length, reviewed } as any, status: 'exported' },
    });
    return {
      success: true,
      data: {
        format: 'csv',
        csv,
        filename: worksheetFilename(filing.jurisdiction, taxYear),
