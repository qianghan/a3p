...
def load_config() -> ProxyConfig:
    ...
    if provider in ("fal-ai", "replicate", "runpod", "gemini"):
        if not api_key:
            raise ConfigError(f"Provider '{provider}' requires: API_KEY")
        # model_id is optional -- can be passed per-request for multi-model
